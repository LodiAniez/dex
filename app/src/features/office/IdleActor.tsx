...
export function IdleActor({ employee, mapHeight, loafing, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    /** Walks `legs`, stopping at the first stop after being recalled. Says whether it got to the end. */
    const walk = async (legs: Leg[], end: Spot, as: AnticKind | "walking", interruptible: boolean) => {
    ...
    void (async () => {
      for (let round = 0; !cancelled; round += 1) {
        ...
        if (arrived && !recalled()) {
          if (plan.act.length > 0) await walk(plan.act, plan.to, plan.kind, true);
          else {
            setDoing(plan.kind);
            await waitUnless(plan.seconds, recalled);
          }

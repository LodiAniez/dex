...
12. End an agent you started yourself (its pane stays, labelled). In Office, **Hire an agent** with that same label: before you can press Hire the form says a pane with that label is still open in Terminal view with no agent in it, and offers the next free one ("lead-2") in one click. With a label an agent's pane holds, it says that instead. A label with a space in it, or longer than 32 characters, is said to be so, with a usable one offered; nothing offered is ever longer than 32.
13. Ask the lead to spawn six agents. The lead's pane keeps its half; the six are stacked beside it in the order they were hired, each readable. Press `Ctrl+Shift+Space` until the grid preset, then have the lead spawn a seventh: the grid grows by one and nothing else moves. Drag a divider by hand, spawn an eighth: your divider stays where you put it, and the eighth joins the strip. `Alt+Shift+Arrow` moves any of them.
14. With four panes, drag one by its header: after a few pixels it dims, and the pane under the pointer shows where it would land - a half for an edge, the whole pane for the middle. Drop on an edge: it sits there, half the space, and the gap it left closes. Drop in the middle: the two trade places. Drag and press Escape, or drop outside any pane: nothing moves. A plain click on a header only focuses the pane. Terminals keep their scrollback and a running agent keeps running through every move.
15. Press ↗ in a running agent's pane header: it opens in its own window with its scrollback, and the main window closes up round the gap. Type in it; drag it to another monitor; resize it. **Dock ↩** puts it back where it was, with everything that happened while it was out. Pop it out again and close it with its own close button: it docks too. With one out, close Dex's main window: everything closes. The last pane in the main window has no ↗.

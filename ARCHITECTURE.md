...
| Claude Code 2.1.273 | The workspace trust dialog ("Is this a project you trust?") is skipped **only** in non-interactive mode — `-p`, or when stdout is not a TTY. An interactive session in a directory it has not seen always stops there. A spawned agent's worktree is always a new directory, so it stops on its first turn. | **Open**: this blocks M7's "starts working with no human input" for worktree spawns. Getting past it means auto-trusting a directory on the owner's behalf, which is their decision. |

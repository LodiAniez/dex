...
- **Pane kinds are checked, not stored blindly.** `pane.create` and `pane.split` take `kind` (`terminal` or `activity`); anything else is refused, because a pane whose kind no renderer understands would be an empty box with no way to tell why. A non-terminal pane mounts no terminal, so no shell is spawned for it.

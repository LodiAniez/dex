...
**Repositories and worktrees.** Register a repo once (`dex repo add <path>`, or `dex repo scan <folder>` to find several). Then any agent — or you — can get a fresh worktree on a new branch, so parallel agents never share a working tree. A worktree is kept apart from everything outside it. When the workspace's root is a plain folder - one that holds your repositories, say - it goes inside the workspace, at `<workspace root>\.dex\worktrees\<repo>\<branch>`. When the root is inside a git checkout (the repository itself, typically), it goes to `%USERPROFILE%\dex\worktrees\<repo>\<branch>` instead: nested in a checkout, an agent would see that project's files around its own - Claude Code reads every `CLAUDE.md` above its folder, Node and Cargo look upwards for their packages and workspaces - and a `git clean -ffdx` there would delete the worktree, uncommitted work and all. `dex worktree remove` finds a worktree wherever it was made, even after its agent has switched branch.
...
worktree_base = "D:/work/worktrees"                # default: %USERPROFILE%\dex\worktrees - for worktrees that cannot go inside their workspace

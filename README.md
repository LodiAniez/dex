...
**Repositories and worktrees.** Register a repo once (`dex repo add <path>`, or `dex repo scan <folder>` to find several). Then any agent — or you — can get a fresh worktree on a new branch, so parallel agents never share a working tree. A worktree is meant to be a place of its own, so where it goes depends on the workspace's root. When the root is a plain folder - one that holds your repositories, say, or the first-run workspace's root, your home folder - it goes inside the workspace, at `<workspace root>\.dex\worktrees\<repo>\<branch>`. When the root is inside a git checkout (the repository itself, typically), it goes to `worktree_base` instead - `%USERPROFILE%\dex\worktrees` unless you set it: nested in a checkout, an agent would see that project's files around its own (Claude Code reads every `CLAUDE.md` above its folder; Node, Cargo and ESLint look upwards for their packages, workspaces and configs), and a `git clean -ffdx` there would delete the worktree, uncommitted work and all. The one thing still above a worktree in a plain root is whatever sits in that root's own top level - a `CLAUDE.md`, a `package.json`, a `Cargo.toml` with a `[workspace]`, which Cargo would take the worktree into - so keep a root that holds worktrees free of those. `dex worktree remove` finds a worktree wherever it was made, even after its agent has switched branch; if a removal fails part-way - on Windows, usually a file something still has open - git stops tracking the worktree but its folder stays, and Dex tells you which folder to check and delete rather than dropping it silently.
...
worktree_base = "D:/work/worktrees"                # default: %USERPROFILE%\dex\worktrees - for workspaces rooted inside a git checkout

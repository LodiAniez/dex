...
**Repositories and worktrees.** Register a repo once (`dex repo add <path>`, or `dex repo scan <folder>` to find several). Then any agent — or you — can get a fresh worktree on a new branch, so parallel agents never share a working tree. It goes inside the workspace it is for, at `<workspace root>\.dex\worktrees\<repo>\<branch>`, beside the context mirror — an agent's checkout lives with the work it is part of. A workspace you make without naming a root is rooted at your home folder, so there that means `%USERPROFILE%\.dex\worktrees`. `worktree_base` (`%USERPROFILE%\dex\worktrees` unless you set it) is only for a worktree with nowhere else to go: no workspace at all (`dex worktree add` without `--workspace`), a root on the distro's side of `\\wsl.localhost` that Windows git cannot work in, or a root that cannot hold the folder.

A workspace's root is usually a repository, so the worktree usually sits inside a checkout of that repository. Dex tells git to look away twice — `.dex\worktrees\.gitignore`, and a `**/.dex/` line in the repository's `.git\info\exclude`, which `git clean` cannot delete — so nothing Dex keeps in a workspace, worktrees or the context mirror, shows in that checkout's `git status` or is taken into a `git add -A` as an embedded repository. A repository that will not take that line is not used: the worktree goes to `worktree_base` instead. Two things to know, and a `README.txt` in the folder says the first: **`git clean -ffdx` in that checkout deletes every worktree under it, uncommitted work and all** (no ignore or `git worktree lock` stops a second `-f`), and anything that looks up the folder tree sees the project around the worktree — Claude Code reads every `CLAUDE.md` above it, Node resolves modules upwards, Cargo walks up to a `[workspace]`, ESLint to its config. `dex worktree prune <repo>` clears out the ones nobody is working in any more - a worktree goes only when no pane has a shell running in it, its tree is clean (untracked files count), and its branch has nothing the main checkout's branch has not; everything else stays, with the reason it stayed, and `--dry-run` says what would go and how much that would free. Ignored files go with the folder, exactly as they do for `git worktree remove`: installed dependencies and build output, and anything else gitignored that lives only there - a hand-written `.env`, most likely - so run it with `--dry-run` first. A worktree whose folder is missing, or unreachable because its distro is not running, is named rather than pruned: Dex will not run `git worktree prune` for you, since a folder that is only unreachable is not a folder that is gone. `dex worktree list <repo> --sizes` measures what each one is holding, which walks every file in every worktree - seconds for a checkout with dependencies installed, and slower still for one on the distro's side of `\wsl.localhost`. After a prune, a pane still pointing at a folder that went has nowhere to start a shell; that is the cost of pruning at all, since keeping every worktree a pane remembers would keep all of them for ever. `dex worktree remove` finds a worktree wherever it was made, even after its agent has switched branch; if a removal fails part-way — on Windows, usually a file something still has open — git stops tracking the worktree but its folder stays, and Dex names the folder to check and delete rather than dropping it silently.
...
worktree_base = "D:/work/worktrees"                # default: %USERPROFILE%\dex\worktrees - only for worktrees with no workspace
...
- **Messages** — directed at one agent, read once. A message to an agent that has finished its task and is sitting at its prompt wakes it: Dex types a one-line prompt into its pane telling it to read its inbox. One sent while it is working waits, and the watchdog wakes it within 15-20 seconds of that turn ending. The sender is told what became of it — woken, waiting for its next turn, waiting on you (its pane is holding a question for you, so nothing may be typed into it), or nothing will read it because that agent has ended — and `dex agent list --json` (and `agents_list`) says how many messages wait unread for each agent. That is how a parent changes a child's task after the fact without stopping it and starting another.
...
`dex agent stop <label>` ends an agent. A label means an agent in your own workspace — the one the pane you ran it in belongs to, or the one `--workspace` names — since two workspaces can each have a `reviewer`; run it from outside a pane with the label live in several, and Dex says which they are instead of guessing. An id needs no workspace. An agent may stop only itself and the agents it spawned — a child told to "stop the others" is refused — while you may stop anyone. If Dex spawned it, its pane is closed with it — Dex made that pane — so a lead told to dismiss its agents does not leave a row of empty shells. An agent you started by typing `claude` keeps its pane: that one is yours.
...
dex worktree   add | remove | list | prune

...
//! `/clear` ends a session and starts another (`SessionEnd`, then
...
async fn a_session_carried_on_moments_after_its_end_brings_the_same_agent_back() {
    ...
    // "clear": the pane's agent that just ended is the one carrying on.
    // Real times: an end is recorded on the daemon's clock.
    let (_dir, state, pane) = pane().await;
    let t = clock::now_millis();
    fire(&state, "session-start", &pane, t, start("s1", "startup")).await;
    let before = only_agent(&state).await;
    fire(&state, "session-end", &pane, t + 1, end("s1", "other")).await;
    fire(
        &state,
        "session-start",
        &pane,
        clock::now_millis() + 3_000,
        start("s2", "compact"),
    )
    .await;
...
#[tokio::test]
async fn quitting_still_ends_the_agent() {
    let (_dir, state, pane) = pane().await;
    fire(&state, "session-start", &pane, 1, start("s1", "startup")).await;
    fire(
        &state,
